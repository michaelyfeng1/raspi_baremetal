...
{
    // struct MailboxFbArray fb_init_tags;

    // fb_init_tags.fb_size_cb.tag.id = FB_SET_PHYSICAL_DIMENSIONS;
    // fb_init_tags.fb_size_cb.tag.buffer_size = 8;
    // fb_init_tags.fb_size_cb.tag.status_code = 0;
    // fb_init_tags.fb_size_cb.MailboxData.fb_size.screen_witdh = 1920;
    // fb_init_tags.fb_size_cb.MailboxData.fb_size.screen_height = 1080;

    // fb_init_tags.fb_virt_size_cb.tag.id = FB_SET_VIRTUAL_DIMENSIONS;
    // fb_init_tags.fb_virt_size_cb.tag.buffer_size = 8;
    // fb_init_tags.fb_virt_size_cb.tag.status_code = 0;
    // fb_init_tags.fb_virt_size_cb.MailboxData.virt_fb_size.virt_screen_witdh = 1920;
    // fb_init_tags.fb_virt_size_cb.MailboxData.virt_fb_size.virt_screen_height = 1080;

    // fb_init_tags.fb_virt_offset_cb.tag.id = FB_GET_VIRTUAL_OFFSET;
    // fb_init_tags.fb_virt_offset_cb.tag.buffer_size = 8;
    // fb_init_tags.fb_virt_offset_cb.tag.status_code = 0;
    // fb_init_tags.fb_virt_offset_cb.MailboxData.fb_screen_virt_offset.x_offset = 0;
    // fb_init_tags.fb_virt_offset_cb.MailboxData.fb_screen_virt_offset.y_offset = 0;

    // fb_init_tags.fb_pitch.tag.id = FB_SET_BITS_PER_PIXEL;
    // fb_init_tags.fb_pitch.tag.buffer_size = 4;
    // fb_init_tags.fb_pitch.tag.status_code = 0;
    // fb_init_tags.fb_pitch.MailboxData.pixel_bpp.bpp = 32;

    // fb_init_tags.fb_rgb_order.tag.id = FB_SET_RGB_ORDER_PER_PIXEL;
    // fb_init_tags.fb_rgb_order.tag.buffer_size = 4;
    // fb_init_tags.fb_rgb_order.tag.status_code = 0;
    // fb_init_tags.fb_rgb_order.MailboxData.rgb_order.order = 0;

    // fb_init_tags.fb_screen_info.tag.id = FB_ALLOCATE_BUFFER;
    // fb_init_tags.fb_screen_info.tag.buffer_size = 8;
    // fb_init_tags.fb_screen_info.tag.status_code = 0;
    // fb_init_tags.fb_screen_info.MailboxData.fb_screen_info.base = 4096;
    // fb_init_tags.fb_screen_info.MailboxData.fb_screen_info.screen_size = 0;

    // fb_init_tags.fb_pixel_depth.tag.id = FB_GET_BYTES_PER_ROW;
    // fb_init_tags.fb_pixel_depth.tag.buffer_size = 4;
    // fb_init_tags.fb_pixel_depth.tag.status_code = 0;
    // fb_init_tags.fb_pixel_depth.MailboxData.pixel_pitch.pitch = 0;

    // u32 mailbox_buffer_size = 3 * sizeof(u32) + sizeof(struct MailboxFbArray);
    // u32 dword_index_count = 0;

    // dword_index_count += mailbox_buffer_size / sizeof(u32);

    // memcpy(&mailbox_buffer[2], &fb_init_tags, sizeof(struct MailboxFbArray));
    ...
    // memcpy(&fb_init_tags, mailbox_buffer + 2,  sizeof(struct MailboxFbArray));


    // u8 *frame_buffer = (u8 *)((u64)(fb_init_tags.fb_screen_info.MailboxData.fb_screen_info.base));

    // fb = (u8*)(P2V((u64)frame_buffer));
